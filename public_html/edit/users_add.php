...
if ($_POST['action'] == "insert") {
	...
	$rs = mysql_query("SELECT user_id FROM wma_users WHERE user_username='".$user_username."'");
	if (mysql_num_rows($rs) > 0) 
		...
		if ($user_type <> "m") { # insert permissions if not Manager 
			...
			while ($rows = mysql_fetch_array($rs)) {
				...
				if ($page_id_content == "y" || $page_id_config == "y") {
					...
					# user_id, page_id, content_page_id, page_config
...
<div id="content">
	...
	<form action="users_add.php" method="post" name="form1" onSubmit="return isReady(form1);">
	...
	<fieldset>
		...
		<dl>
		  ...
		  <dd><div id="pages"><?
			...
			while ($rows = mysql_fetch_array($rs)) {
				echo "<input name=\"page_id".$rows[0]."\" type=\"checkbox\" id=\"select_page\" value=\"y\" ".((${'page_id'.$rows[0]} == "y")?" checked=\"checked\"":"")." /> 
				<input name=\"page_id_config".$rows[0]."\" type=\"checkbox\" id=\"select_page\" value=\"y\" ".((${'page_id_config'.$rows[0]} == "y")?" checked=\"checked\"":"")." /> ".
				$rows[1]."<br />\n";
			}
	...
	<? } ?>
	<div class="buttonarea"><input type="submit" value="Insert" class="button" /></div>
	</form>
